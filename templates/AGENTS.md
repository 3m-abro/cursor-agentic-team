# AGENTS.md — Agentic Team (copy to project root)
...
| Skill | Points at |
|-------|-----------|
| `design-ui-ux-pro-max` | `~/.cursor/skills/ui-ux-pro-max/` |
| `design-taste` | `~/.agents/skills/taste-design/` (fallback `~/.claude/skills/taste-design/`) |
| `design-frontend` | `~/.claude/skills/frontend-design/` |
| `design-transitions` | GSAP `gsap-scrolltrigger` + `gsap-core` (+ ui-ux-pro-max motion taste) |
| `design-web-artifacts` | `~/.cursor/skills-cursor/canvas/` + Stitch skills |
| `design-brand` | `~/.claude/skills/brand/` |

## MARKETING skill map (Phase 2)

| Skill | Points at |
|-------|-----------|
| `marketing-copywriting` | `~/.claude/skills/copywriting/` |
| `marketing-ai-seo` | seo-audit + schema-markup + content-strategy |
| `marketing-cro` | `~/.claude/skills/page-cro/` |
| `marketing-ad-creative` | banner-design + ogilvy |
| `marketing-customer-research` | NEW thin procedure (content-strategy + optional startup-analyst Task) |
...
| `social-voice-builder` | `~/.claude/skills/brand/` (voice) |
...
## Parallel patterns (Multitask Mode)
...
1. Missing tool/MCP access → stop, tell user what's blocked.
